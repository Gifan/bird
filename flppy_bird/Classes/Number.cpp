...
{
	this->m_numberSeries = Vector<SpriteFrame*>(10);
	return true;
}
...
void NumberSeries::loadNumber(const char* fmt, int base /* = 0 */)
{
	for (int i = base; i < 10+base; i++)
	{
		...
		auto frame = AtlasLoader::getInstance()->getSpriteFrameByName(
			filename);
		m_numberSeries.pushBack(frame);
	}
}

SpriteFrame* NumberSeries::at(int index) 
{
	return this->m_numberSeries.at(index);
}
